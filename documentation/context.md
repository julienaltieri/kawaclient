...
| [`Spec - stream prediction.md`](<../src/processors/streamPredictor/Spec - stream prediction.md>) | The v2 stream model, as a decomposition into six sub-problems: which accounts a stream moves through, its rhythm, the shape of a cycle, the amount, and the schedule of money events that follows. Built and audited against the captured portfolio. |
| [`Spec - balance prediction.md`](<../src/processors/balancePrediction/Spec - balance prediction.md>) | One ledger per account, from the stream predictions plus the transactions that have happened, and the balance that ledger implies. Built. Reachable only from the sandbox, where it is offered beside the shipped forecaster for comparison — see [`bank-balance.md`](bank-balance.md). |
| [`bank-balance.md`](bank-balance.md) | Page three: the balance to date and ahead. The largest document here and the one under active change — read §2f first, it supersedes §2. |
