...
| [`Spec - stream prediction.md`](<../src/processors/streamPredictor/Spec - stream prediction.md>) | The v2 stream model, as a decomposition into six sub-problems. A specification under review. Stages 1–4 are implemented beside it and audited against the captured portfolio; nothing is wired to the balance until the exit criteria are met. |
| [`Spec - balance prediction.md`](<../src/processors/balancePrediction/Spec - balance prediction.md>) | What an account's balance will be between now and a date, as a decomposition into four sub-problems. A specification under review, nothing implemented. Sibling to the stream prediction spec, which it takes as input. |
