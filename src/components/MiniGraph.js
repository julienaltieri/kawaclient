...
export default class MiniGraph extends GenericChartView{
	...
	render(){
	    ...
	    <V.VictoryChart scale={{ x: "time", y:"linear" }} domain={this.getDomain()} height={this.style.chartHeight} width={this.style.chartWidth} padding={this.style.chartPadding}
	    				...
	    				style={{touchAction:"auto"}} voronoiBlacklist={["lineChart-1","lineChart-2","lineChart-3"]}
			...
			<V.VictoryAxis style={{axis:{opacity:1,stroke:DesignSystem.getStyle().bodyTextSecondary}}}
			 ...
			 />
			{this.props.stream.expAmountHistory?.map((h,i) => <V.VictoryLine name="lineChart-1" key={1000+i} data={[{x:h.startDate.getTime(),y:-0.7*domainAxisVerticalPadding},{x:h.startDate.getTime(),y:0.7*domainAxisVerticalPadding}]} style={{data: {stroke:DesignSystem.getStyle().bodyTextSecondary,strokeWidth:1}}}/>) }
