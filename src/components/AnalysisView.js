...
export class EndOfPeriodProjectionGraph extends GenericChartView{
		...
		let res = {
			plotList: [
				new SeriesDescriptor({ name:"savings", chartContext:chartContext,
					...
					target: this.timeAxis.map((d,i) => utils.sum(this.timeAxis.slice(0,i+1).map((dd,j) => -this.props.savingsAnalysis.getExpectedAmountAtDateForPeriod(Period.monthly.nextDate(dd),Period.monthly)))),
				...
				new SeriesDescriptor({ name:"expenses", chartContext:chartContext,
					...
					target: this.timeAxis.map((d,i) => utils.sum(this.timeAxis.slice(0,i+1).map((dd,j) => this.props.expenseAnalysis.getExpectedAmountAtDateForPeriod(Period.monthly.nextDate(dd),Period.monthly)))),
	...
	render(){
		...
		{/*The tile fills its carousel page exactly: the page is already held off the container's edge by the
		   deck's own padding, so a tile that also inset itself counted that space twice.
		   boxSizing:border-box is required, not decorative - this codebase sets box-sizing per component and
		   has no global rule, so height:100% PLUS padding made the tile 2rem taller than the page it sits in,
		   and the deck's overflow:hidden cut that 2rem off the bottom along with two rounded corners.*/}
